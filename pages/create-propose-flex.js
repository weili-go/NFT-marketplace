...
export default function Home() {
  ...
  const [formInput, updateFormInput] = useState({ targets: '', calldatas: '', description: '', funcs: '' })
  ...
  async function propose() {
    const { targets, description, calldatas } = formInput

    //
    if (!targets || !calldatas || !description) {
    ...
    try {
      ...
      const targets_p = targets.split('|');
      const funcs = calldatas.split('|');
      console.log(funcs)
  ...
  return (
    <div className="flex justify-center">
      <div className="w-1/2 flex flex-col pb-12">
        <input
          placeholder="target contract1 | target contract2 | target contract3 | ..."
        ...
        />
        <p className="text-1xl my-1 text-yellow-400 font-bold">description file(this will be saved in IPFS): </p>
        <input
          type="file"
          name="NFT"
          className="my-4"
